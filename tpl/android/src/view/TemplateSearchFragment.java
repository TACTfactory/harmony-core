 ...
 */
public class ${curr.name}SearchFragment extends Fragment implements OnClickListener {
	/* Model data */
	...
	protected void init${relation.name?cap_first}Dialog(List<${relation.relation.targetEntity}> list){
		...
		builder.setTitle("Select ${relation.name}")
				.setSingleChoiceItems(listAdapter, 0, new DialogInterface.OnClickListener(){
		            ...
		            @Override
		            public void onClick(DialogInterface dialog, int id) {
		            	${curr.name}SearchFragment.this.selected${relation.name?cap_first} = Integer.parseInt(listAdapter[((AlertDialog)dialog).getListView().getCheckedItemPosition()]);
		            }
		        }).setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
		            @Override
		            public void onClick(DialogInterface dialog, int id) {

...
<#if (curr.fields?size > 0 || curr.inheritance??)>
    ...
    public static final String TABLE_NAME = <#if singleTabInheritance && !isTopMostSuperClass>${curr.inheritance.superclass.name?cap_first}Contract.TABLE_NAME<#else>"${curr.tableName}"</#if>;
    /** Global Fields. */

...
public class FixtureGenerator extends BaseGenerator<IAdapter> {
    ...
    public final void load() {
        List<IUpdater> updaters = this.getAdapter().getAdapterProject().getFixtureAssets();
    ...
    public final void init(final boolean force) {
         try {
            final String fixtureType = ((FixtureMetadata) this.getAppMetas().getOptions().get("fixture")).getType();

            final FixtureMetadata meta = (FixtureMetadata) this.getAppMetas().getOptions().get(FixtureMetadata.NAME);

            List<IUpdater> updaters = this.getAdapter().getAdapterProject().getFixtureLibraries(meta.getType());
            this.processUpdater(updaters);

            updaters = this.getAdapter().getAdapterProject().getFixtureFiles(force);
            this.processUpdater(updaters);
            ...
            Iterable<EntityMetadata> entities = this.getAppMetas().getEntities().values();
            ...
            for (final EntityMetadata classMeta : entities) {
                if (classMeta.hasFields() && !classMeta.isInternal()) {
                    ...
                    updaters = this.getAdapter().getAdapterProject()
                            .getFixtureEntityDefinitionFiles(fixtureType, classMeta);
                    this.processUpdater(updaters);
    ...
    public final void purge() {
        for (final ClassMetadata classMeta : this.getAppMetas().getEntities().values()) {
    ...
    private void removeSource(final String fileName) {
        final String fullFilePath = this.getAdapter().getAssetsPath() + fileName;
        final File file = new File(fullFilePath);

        if (file.exists() && !file.delete()) {
            ConsoleUtils.displayError(new Exception("Couldn't delete file " + file.getPath()));
        }
    }
}

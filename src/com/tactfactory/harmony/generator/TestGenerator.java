...
public class TestGenerator extends BaseGenerator<IAdapter> {
    ...
    public final void generateAll() {
        ...
        List<IUpdater> updaters = this.getAdapter().getAdapterProject().getTestFiles();
    ...
    private void generateEntityTest(EntityMetadata entity) {
        // Info
        ConsoleUtils.display(">>> Generate Repository test for " + this.getDatamodel().get(TagConstant.CURRENT_ENTITY));

        try {
            List<IUpdater> updaters = this.getAdapter().getAdapterProject().getTestEntityFiles(entity);
            this.processUpdater(updaters);

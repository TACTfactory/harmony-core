...
public class GitUtils {
    ...
    public static String getRef(String repositoryPath) {
        String result = null;

        try {
            File gitFolder = new File(repositoryPath, ".git");

            if (gitFolder.exists()) {
                if (gitFolder.isFile()) {
                    // If current working dir is a submodule, get repository folder from main repo
                    String content = FileUtils.readFileToString(gitFolder);
                    repositoryPath = content.replace("gitdir:", "").replace("\n", "");
                }

                Git libModule = Git.open(new File(repositoryPath));
                Repository repository = libModule.getRepository();
                RevWalk walk = new RevWalk(repository);
                ObjectId resolve = repository.resolve(Constants.HEAD);
                result = walk.parseCommit(resolve).name();
            }
        } catch (IOException e) {
            ConsoleUtils.displayWarning(e.getMessage());
        }
